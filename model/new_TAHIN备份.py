...
class DCCF(nn.Module):
    ...
    def inference(self):
        ...
        self.ua_embedding = 0.5 * self.ua_embedding + 0.5 * h2[self.userkey]
        self.ia_embedding = 0.5 * self.ia_embedding + 0.5 * h2[self.itemkey]
    ...
    def forward(self, users, pos_items, neg_items):
        ...
        # embeddings
        u_embeddings_pre = self.feature_dict[self.userkey][users]
        pos_embeddings_pre = self.feature_dict[self.itemkey][pos_items]
        neg_embeddings_pre = self.feature_dict[self.itemkey][neg_items]
        ...
        cl_loss = self.ssl_reg * self.cal_ssl_loss(users, pos_items, gnn_embeddings, int_embeddings, gaa_embeddings,
                                                   iaa_embeddings)
    ...
    def getUsersRating(self, user_idx):
        self.inference()
        u_embeddings = self.ua_embedding[user_idx]
        i_embeddings = self.ia_embedding
        rating = torch.matmul(u_embeddings, i_embeddings.T)
        return rating
...
class TAHIN(nn.Module):
    ...
    def forward(self, user_idx, item_idx, neg_item_idx):
        mf_loss, emb_loss, cen_loss, cl_loss = self.DCCF(user_idx, item_idx, neg_item_idx)
    ...
    def getUsersRating(self, user_idx):
        ...
        rating = self.DCCF.getUsersRating(user_idx)
        return rating