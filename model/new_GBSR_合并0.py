...
class TAHIN(nn.Module):
    def __init__(self, g, args):
        ...
        for key in [self.user_key, self.item_key]:
            ...
            for meta_path_pattern in self.meta_path_patterns[key]:
                rows = self.UI_interaction_rows[key][str(meta_path_pattern)]
                cols = self.UI_interaction_cols[key][str(meta_path_pattern)]
                values = self.UI_interaction_values[key][str(meta_path_pattern)]
                self.merged_rows[key] = rows
                self.merged_cols[key] = cols
                self.merged_values[key] = values
            ...
            shape = torch.Size(self.normalize_UI_adj[key][str(self.meta_path_patterns[key][0])].shape)
            # 构建合并后的稀疏张量
    ...
    def graph_reconstruction(self, mask_ratio=0.1):
        ...
        for key in self.meta_path_patterns.keys():
            ...
            # 随机选择要掩码的边的索引
            mask_indices = torch.randperm(num_edges)[:num_masked_edges]

            # 获取随机选择的边的 row 和 col
            masked_row = row[mask_indices]
            masked_col = col[mask_indices]
            values = self.merged_values[key][mask_indices]
