...
####################################################
class Contrast_User(nn.Module):
    def __init__(self, g, args):
        super(Contrast_User, self).__init__()
        ...
        self.user_meta_path_patterns = args.meta_path_patterns[user_key]
        self.proj = nn.Sequential(
            nn.Linear(in_size, hidden_dim),
            nn.ELU(),
            nn.Linear(hidden_dim, hidden_dim)
        )
        self.pos = self.find_pos(g, user_key).to(self.device)
        for model in self.proj:
            if isinstance(model, nn.Linear):
                nn.init.xavier_normal_(model.weight, gain=1.414)

    def find_pos(self, g, user_key):
    ...
    def forward(self, z_mp, z_sc):
        z_proj_sc = self.proj(z_sc)  # n_batchUser x emb_size
        z_proj_mp = self.proj(z_mp)
        matrix_mp2sc = self.sim(z_proj_mp, z_proj_sc)  # n_batchUser x n_batchUser
