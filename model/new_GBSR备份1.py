        ...
        for i in range(self.han_layers):
            for key in self.meta_path_patterns.keys():
                ...
                for value in self.meta_path_patterns[key]:
                    value = str(value)
                    if i == 0:
                        embeddings.append(self.GCN_layer(self.feature_dict[key], self.normalize_UI_adj[key][value]))
                    else:
                        embeddings.append(self.GCN_layer(h2[key], self.normalize_UI_adj[key][value]))
                # 对所有元路径的嵌入进行平均
