# Amazon: best epoch: 117, bset_recall: 0.10106, best_ndcg: 0.086508
import dgl
import torch
import numpy as np
import torch_sparse
...
class TAHIN(nn.Module):
    ...
    def create_sparse_adjaceny(self):
        ...
        temp_col_idx = [idx + self.n_users for idx in col_idx]  # 将列索引转换成物品索引，确保它们在用户索引之后
        ...
        self.joint_adjaceny_matrix = torch.sparse_coo_tensor(ui_index, value, (self.n_nodes, self.n_nodes))
    ...
    def forward(self, user_idx, item_idx, neg_item_idx):
        # DCCF嵌入
        ua_embedding, ia_embedding = self.DCCF(self.G_indices, self.G_values, self.feature_dict)
    ...
    def predict(self, user_idx, item_idx):
        ...
        user_emb = torch.cat((user_emb, user_embeddings_edge_drop, user_embeddings_node_drop), dim=-1)
        item_emb = torch.cat((item_emb, item_embeddings_edge_drop, item_embeddings_node_drop), dim=-1)
