# best epoch: 38, bset_recall: 0.10855, best_ndcg: 0.091357
# 去掉gaa和iaa recall可达0.102
import dgl
import torch
import numpy as np
import torch_sparse
...
class DCCF(nn.Module):
    ...
    def forward(self, feature_dict):
        ...
        for i in range(0, self.n_layers):
            ...
            all_embeddings.append(gnn_layer_embeddings + int_layer_embeddings + all_embeddings[i] + gaa_layer_embeddings + iaa_layer_embeddings)
...
class TAHIN(nn.Module):
    def __init__(self, g, args):
        ...
        self.pre_encoder = nn.ModuleDict({
            name: nn.Linear(in_size, out_size, bias=False) for name in g.ntypes
        })
    ...
    def forward(self, user_idx, item_idx, neg_item_idx):
        # feature_dict = {ntype: self.pre_encoder[ntype](self.feature_dict[ntype]) for ntype in self.g.ntypes}
        feature_dict = self.feature_dict
        ua_embedding, ia_embedding, gnn_embeddings, int_embeddings = self.DCCF(feature_dict)
        # metapath-based aggregation, h2
        h2 = {}
        for key in self.meta_path_patterns.keys():
            for i in range(self.han_layers):
                if i == 0:
                    h2[key] = self.hans[key](self.g, feature_dict[key])
                else:
        ...
        bpr_loss_edge_drop, bpr_loss_node_drop, \
            ...
            = self.Contrast_IB(user_emb, item_emb,
                               feature_dict[self.user_key],
                               feature_dict[self.item_key],
    ...
    def predict(self, user_idx, item_idx):
        ...
        user_embeddings_edge_drop, item_embeddings_edge_drop, user_embeddings_node_drop, item_embeddings_node_drop \
            = self.Contrast_IB.predict(user_emb, item_emb, feature_dict[self.user_key],
                                       feature_dict[self.item_key], user_idx, item_idx)
