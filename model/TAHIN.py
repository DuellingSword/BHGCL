...
#      def __init__(self, g, userkey, itemkey, in_size, out_size, layer_num_heads, dropout):
...
#                  GATConv(in_size, out_size, 1,
#                          0.8, 0.8, activation=F.elu,
...
#             embedding_u.append(self.gat_layers_node[i](new_g, new_h).flatten(1)[:self.unum, :])
#
...
#             new_g = dgl.to_homogeneous(new_g)
#             new_h = torch.cat((h[srctype].data,h[dsttype].data),0)
#             embedding_i.append(self.gat_layers_node[k](new_g, new_h).flatten(1)[:self.inum, :])
...
# Relational neighbor aggregation
class RelationalAGG(nn.Module):
    def __init__(self, g, in_size, out_size, dropout=0.7):
        super(RelationalAGG, self).__init__()
