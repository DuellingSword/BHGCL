...
# Semantic attention in the metapath-based aggregation
...
class BHGCL(nn.Module):
    def __init__(self, g, args):
        super(BHGCL, self).__init__()
        self.g = g
    ...
    def forward(self, user_idx, item_idx, neg_item_idx):
        ua_embedding, ia_embedding, gnn_embeddings, int_embeddings = self.Disen(self.feature_dict)
    ...
    def predict(self, user_idx, item_idx):
        ua_embedding, ia_embedding, gnn_embeddings, int_embeddings = self.Disen(
            self.feature_dict)
        # metapath-based aggregation, h2
        h2 = {}
        for key in self.meta_path_patterns.keys():
