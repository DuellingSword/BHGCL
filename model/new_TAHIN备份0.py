# best epoch: 149, bset_recall: 0.10305, best_ndcg: 0.087585
import dgl
import torch
import numpy as np
import torch_sparse
...
class TAHIN(nn.Module):
    ...
    def forward(self, user_idx, item_idx, neg_item_idx):
        ...
        # 计算对比损失
        cl_user_loss = self.Contrast_User(ua_embedding, h2[self.user_key])
        cl_item_loss = self.Contrast_Item(ia_embedding, h2[self.item_key])
