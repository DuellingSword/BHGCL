...
class GraphAttention(nn.Module):
    ...
    def norm(self, edges):
        # normalize attention
        a = edges.data['a'] / (edges.dst['z'] + 1e-10)
        return {'a' : a}

    def loop(self, edges):
        # set attention to itself as 1
        # return lambda edges: {'a': torch.ones(edges.shape[0], 3 , 1)}
...
class GAT(nn.Module):
    def __init__(self,
        ...
        self.gat_layers.append(GraphAttention(
            in_dim, num_hidden, heads[0], feat_drop, attn_drop, alpha,bias_l0, False, l0=l0, min=0))
        # hidden layers
    ...
    def forward(self, g, inputs):
        ...
        # h = self.activation(h.flatten(1).clamp(min=0))
